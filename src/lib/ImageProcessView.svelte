  ...
  {:else}
    <div class="workspace">
      <aside class="controls">
        ...
        <section><strong>{t("旋转与翻转", "Rotate and flip")}</strong><span class="subtle">{t("会作用于导出结果", "Applies to the exported result")}</span><div class="button-row"><button class="dbx-btn" type="button" onclick={() => rotate(-90)}>↶ 90°</button><button class="dbx-btn" type="button" onclick={() => rotate(90)}>↷ 90°</button><button class="dbx-btn" class:active={flipHorizontal} type="button" onclick={() => (flipHorizontal = !flipHorizontal)}>{t("水平翻转", "Flip H")}</button><button class="dbx-btn" class:active={flipVertical} type="button" onclick={() => (flipVertical = !flipVertical)}>{t("垂直翻转", "Flip V")}</button></div><span class="subtle">{t("当前旋转", "Rotation")}: {rotation}°</span></section>
...
<style>
  ...
  .check { flex-direction: row !important; align-items: center; }.button-row { display: flex; flex-wrap: wrap; gap: 6px; }.button-row .active { border-color: var(--color-primary); color: var(--color-primary); background: color-mix(in srgb, var(--color-primary) 8%, var(--color-background)); }.subtle { color: var(--color-muted-foreground); font-size: 11px; }.crop-rotate-row { display: flex; flex-direction: column; gap: 6px; padding-top: 2px; }
