...
const rows = [
  ...
  // —— 转换计算 ——
  ["timestamp", "convert", "时间戳", "Timestamp", "time-convert", [
    "duration", "unix", "timestamp", "time", "beijing", "epoch",
    "时间戳转换", "北京时间", "时间差", "时长", "utc",
  ]],
  ["color", "convert", "颜色转换", "Color convert", "color-convert", [
    "hex", "rgb", "hsl", "hsv", "cmyk", "colour", "eyedropper", "picker",
    "取色", "色环", "颜色",
  ]],
  ["cron", "convert", "Cron 表达式", "Cron expression", "cron", [
  ...
  ["network-calc", "convert", "网络 / CIDR", "Network / CIDR", "network-calc", [
    "ip", "ipv4", "ipv6", "cidr", "子网", "子网切分", "子网掩码", "广播地址",
    "网络地址", "ip转整数", "网段", "subnet",
  ]],

  // —— 文本工具 ——
  ["whitespace", "text", "文本处理", "Text tools", "whitespace", [
  ...
  ["regex", "text", "正则测试", "Regex tester", "regex", [
    "正则", "正则表达式", "regexp", "pattern", "正则匹配", "正则替换",
  ]],
  ["diff", "text", "文本对比", "Text diff", "diff", [
    "diff", "对比", "差异", "compare", "文本差异",
  ]],
  ["markdown", "text", "Markdown", "Markdown", "markdown", [
    "md", "markdown预览", "markdown渲染", "预览markdown",
  ]],
  ["unicode-inspect", "text", "Unicode 检查", "Unicode inspector", "unicode-inspect", [
    "码位", "code point", "codepoints", "字符检查", "emoji", "unicode",
  ]],

  // —— 安全加密 ——
  ["hash", "security", "哈希校验", "Hash & checksum", "hash", [
  ...
  ["aes", "security", "对称加密", "Symmetric cipher", "aes", [
    "sm4", "aes", "gcm", "cbc", "ecb", "iv", "nonce", "aad", "对称", "加解密",
  ]],
  ["hmac-sha256", "security", "HMAC", "HMAC", "hmac", [
    "hmac-sm3", "hmac", "hmac-sha1", "hmac-sha384", "hmac-sha512", "消息认证",
  ]],
  ["rsa", "security", "非对称加密", "Asymmetric cipher", "rsa", [
    "sm2", "rsa", "oaep", "pkcs1", "非对称", "公钥加密",
  ]],
  ["totp", "security", "TOTP", "TOTP", "totp", [
    "otp", "otpauth", "双因素", "二步验证", "动态口令", "验证码", "2fa", "mfa",
  ]],
  ["cert", "security", "证书与 SSH", "Certificate & SSH", "cert", [
    "ssh-fingerprint", "x509", "pem", "证书", "指纹", "ssh公钥", "certificate", "san",
  ]],
  ["jwk", "security", "JWK / JWKS", "JWK / JWKS", "jwk", [
    "jwk", "jwks", "json web key", "JSON Web Key", "jwk转pem", "pem转jwk",
  ]],
  ["symmetric-key", "security", "对称密钥", "Symmetric key", "symmetric-key", [
    "aes-key", "hmac-key", "生成密钥", "密钥生成", "随机密钥",
  ]],
  ["key-pair", "security", "密钥对", "Key pair", "keypair", [
    "keypair", "rsa", "sm2", "生成密钥对", "公私钥", "公钥", "私钥",
  ]],
  ["xor", "security", "XOR（调试）", "XOR (debug)", "xor", [
    "异或", "xor加密", "debug xor",
  ]],

  // —— 生成器 ——
  ["uuid", "generate", "唯一 ID", "Unique ID", "unique-id", [
    "ulid", "nanoid", "uuid", "guid", "唯一标识", "生成id", "生成uuid",
  ]],
  ["password", "generate", "随机密码", "Random password", "password", [
    "random-bytes", "密码生成", "随机字节", "强密码", "password generator",
  ]],
  ["qrcode", "generate", "二维码 / 条码", "QR & barcodes", "qrcode", [
    "qr", "barcode", "汉信", "pdf417", "data matrix", "svg",
    "批量识别", "扫码", "生成二维码", "识别二维码",
  ]],
  ["lorem", "generate", "重复文本", "Repeated text", "lorem", [
    "ipsum", "placeholder", "dbx", "推广文案", "自定义内容", "占位文本", "lorem ipsum",
  ]],

  // —— 图片 ——
  ["image-process", "image", "图片处理", "Image editor", "image-process", [
    "图片压缩", "裁剪", "缩放", "旋转", "翻转", "水印", "格式转换",
    "jpeg", "png", "webp", "jpg", "图片编辑", "compress image",
  ]],
  ["image-generate", "image", "占位图", "Placeholder image", "image-generate", [
